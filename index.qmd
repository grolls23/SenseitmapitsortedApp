...
The data is used to develop an indicative flood risk map for regions and sub-regions in Pakistan. Risk commonly defined as function of hazard, exposure and vulnerability. The baseline assumption of this analysis is that areas that have been flooded in the past are expected to flood in the future. Hence, using a cumulative analysis of flood events between 2000 and 2018, the number of flood events is used as a proxy for exposure. A vulnerability index is calculated as a product of the raw population count and the number of flood occurrences
